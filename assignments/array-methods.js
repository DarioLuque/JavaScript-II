...
let ticketPriceTotal = runners.reduce(function(accumlator, currentValue){
    return accumlator + currentValue.donation;},0)
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1: find people who dontated less than 50.

let smallDonation = runners.filter(function(money){
    return money.donation < 50;
   
    })
   console.log(smallDonation);
// Problem 2: list all small shirts
const size = runners.filter(runner => runner.shirt_size === 'S');

console.log(size);

// Problem 3 return all runners first name and sizes.
let nameSize= [];
runners.map(function(element){
    nameSize.push(element.first_name + " " + element.shirt_size);
})
